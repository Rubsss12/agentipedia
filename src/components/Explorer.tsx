...
import { CODA, CODA_ORDER, codaQuadrant, codaRank, type CodaKey } from "@/lib/coda";
...
type SortKey = "az" | "newest" | "confidence" | "coda";
...
const STR = {
  en: {
    ...
    coda: "CODA™ quadrant", allCoda: "All quadrants", codaSort: "CODA™ maturity (level × scope)",
    ...
    az: "Company A to Z", newest: "Newest first", conf: "Confidence first",
  ...
  fr: {
    ...
    coda: "Quadrant CODA™", allCoda: "Tous les quadrants", codaSort: "Maturité CODA™ (niveau × portée)",
    ...
    az: "Entreprise A à Z", newest: "Plus récents", conf: "Confiance d'abord",
...
function matches(e: Entry, f: Filters): boolean {
  ...
  if (f.coda && codaQuadrant(e.coda) !== f.coda) return false;
...
export default function Explorer({ entries }: { entries: Entry[] }) {
  ...
  useEffect(() => {
    const on = (e: Event) => setF((prev) => ({ ...prev, coda: (e as CustomEvent).detail as CodaKey }));
    window.addEventListener("agentipedia:coda", on);
    return () => window.removeEventListener("agentipedia:coda", on);
  }, []);
  ...
  const results = useMemo(() => {
    ...
    if (sort === "newest")
      ...
      list.sort((a, b) => codaRank(a.coda) - codaRank(b.coda) || a.company.localeCompare(b.company));
  ...
  if (f.coda) chips.push({ key: "coda", label: `${f.coda} · ${lang === "fr" ? CODA[f.coda].fr : CODA[f.coda].en}` });
  if (f.regulation) {
      ...
      <div className="rounded-2xl border border-lavender-line bg-lilac-soft p-4 md:p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          ...
          <Select
            label={t.coda}
            value={f.coda}
            onChange={(v) => set({ coda: v as Filters["coda"] })}
            options={CODA_ORDER.map((k) => ({ value: k, label: `${k} · ${lang === "fr" ? CODA[k].fr : CODA[k].en}` }))}
            anyLabel={t.allCoda}
          />
          <Select
            label={t.regulation}
